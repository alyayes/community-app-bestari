          ...
          for (let i = 0; i < foundPositions.length; i++) {
            ...
            if (curr.key === 'title') {
              ...
              if (times && times.length >= 2) {
                ...
                setAgTime(`${sPad} - ${ePad} WIB (${period})`);
              } else if (times && times.length === 1) {
                let s = times[0].includes(':') ? times[0] : `${times[0].padStart(2, '0')}:00`;
                const sPad = s.padStart(5, '0');
                setAgStartTime(sPad);
                const period = to12HourPeriod(sPad).period;
                setAgTime(`${sPad} WIB (${period})`);
              }
    ...
    <div className="min-h-screen bg-[#FAF6EE] flex flex-col font-sans text-[#2C4219]">
      ...
      <main className={`flex-1 flex flex-col min-w-0 transition-all duration-300 pb-16 md:pb-0 ${isSidebarAdminCollapsed ? 'md:pl-20' : 'md:pl-64'}`}>
        ...
        <div className="p-6 sm:p-8 lg:p-10 overflow-y-auto w-full">
          ...
          {activeTab === 'cms' && (
            <div className="space-y-6 w-full max-w-7xl font-['Poppins']">
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h1 className="font-['Poppins'] font-extrabold text-2xl sm:text-3xl text-[#2C4219] tracking-tight">
                    Kelola Konten &amp; Tampilan
                  </h1>
                  <p className="font-['Poppins'] text-sm sm:text-base text-[#5C5246] font-medium mt-1">
                    Atur teks utama, gambar banner, logo, dan tampilan visual halaman depan &amp; akun.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={handleSaveCms as any}
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-[#2C4219] hover:bg-[#1E2E11] text-white font-['Poppins'] font-bold text-sm transition-all shadow-md hover:shadow-lg active:scale-95 shrink-0 cursor-pointer"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                {([
                  { key: 'identitas', label: 'Identitas Web', icon: Type, desc: 'Logo & Nama Situs' },
                  { key: 'landing', label: 'Halaman Utama', icon: Home, desc: 'Banner Hero & Foto' },
                  { key: 'login', label: 'Halaman Login', icon: LogIn, desc: 'Sambutan & Banner' },
                  { key: 'register', label: 'Halaman Register', icon: UserPlus, desc: 'Ajakan Bergabung' },
                  { key: 'footer', label: 'Pengaturan Footer', icon: LayoutGrid, desc: 'Kebijakan & Tautan' }
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-['Poppins'] font-bold text-sm sm:text-base leading-tight">{label}</p>
              ...
              <div className={`grid grid-cols-1 ${cmsActivePage === 'footer' ? '' : 'lg:grid-cols-2'} gap-6 items-start`}>
                {/* LEFT: Editor */}
                <div className="bg-white p-6 sm:p-7 rounded-3xl border border-[#E6E1D5] shadow-sm space-y-6 font-['Poppins']">
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 font-bold text-sm text-[#2C4219]">
                          <Type className="w-4 h-4 text-[#A8B774]" /> Nama Website / Komunitas
                        </label>
                        <input
                          type="text"
                          value={cmsWebName}
                          ...
                          className="w-full p-3.5 rounded-xl border border-[#E6E1D5] bg-[#FAF6EE]/50 text-sm font-semibold text-[#2C4219] focus:outline-none focus:border-[#2C4219] focus:bg-white focus:ring-2 focus:ring-[#2C4219]/10 transition-all"
                        />
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {cmsWebLogo && (
                          <div className="relative w-32 h-32 rounded-xl overflow-hidden border border-[#A8B774]/60 bg-white mb-2">
                            ...
                            <button
                              ...
                              title="Hapus gambar"
                              className="absolute top-1 right-1 w-6 h-6 rounded-full bg-black/60 hover:bg-red-600 text-white flex items-center justify-center transition-colors shadow-sm cursor-pointer"
                            >
                        ...
                        {!cmsWebLogo && (
                          <input
                            ...
                            className="w-full p-3 rounded-xl border border-[#E6E1D5] text-xs font-medium focus:outline-none focus:border-[#2C4219] bg-[#FAF6EE]/50"
                          />
                        )}
