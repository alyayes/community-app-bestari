    ...
    <div className="min-h-screen bg-[#FAF6EE] flex flex-col font-sans text-[#2C4219]">
          ...
          <div className="mt-auto space-y-2 pt-4 border-t border-[#E6E1D5]">
            {setAppMode && (
              ...
              >
                ...
                {isSidebarAdminCollapsed ? (
                  <div className={`w-4 h-4 rounded-full transition-colors duration-300 ${isSwitchingMode ? 'bg-[#2C4219]' : 'bg-[#D1D5DB]'}`} />
                ) : (
                  <div className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors duration-300 ${isSwitchingMode ? 'bg-[#2C4219]' : 'bg-[#D1D5DB]'}`}>
                    <span className={`pointer-events-none absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-all duration-300 ${isSwitchingMode ? 'left-[18px]' : 'left-0.5'}`} />
                  </div>
                )}
              </button>
            )}
      ...
      <main className={`flex-1 flex flex-col min-w-0 transition-all duration-300 pb-20 md:pb-0 ${isSidebarAdminCollapsed ? 'md:pl-20' : 'md:pl-64'}`}>
        ...
        <div className="p-6 sm:p-8 lg:p-10 overflow-y-auto w-full">
          ...
          {activeTab === 'informasi' && (
            previewArticle ? (
              <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[#E6E1D5] shadow-xs">
                <button
                  onClick={() => setPreviewArticle(null)}
                  className="mb-6 text-[#7A7062] hover:text-[#2C4219] font-bold text-xs flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-[#FAF6EE] transition-colors -ml-3"
                >
                  <ArrowLeft className="w-4 h-4" /> Kembali ke Daftar
                </button>
                <div className="mb-4">
                  <span className={`inline-block px-2.5 py-1 rounded text-[10px] font-black tracking-wider uppercase mb-3 ${getCategoryColor(previewArticle.category || '')}`}>
                    {previewArticle.category || 'TIPS'}
                  </span>
                  <h2 className="text-2xl sm:text-3xl font-title font-bold text-[#2C4219] leading-tight mb-3">
                    {previewArticle.title}
                  </h2>
                  <div className="flex items-center gap-2 text-xs text-[#7A7062] font-semibold">
                    <span>{previewArticle.date || previewArticle.createdAt}</span>
                    <span>•</span>
                    <span>Oleh {previewArticle.authorName || 'Admin'}</span>
                  </div>
                </div>
                <div className="prose prose-sm prose-p:text-[#433A30] prose-p:leading-relaxed max-w-none whitespace-pre-wrap font-medium">
                        ...
                        <div className="flex items-start gap-4 flex-1">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-black tracking-wider uppercase ${(art as any).status === 'Draft' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                                }`}>
          ...
          {activeTab === 'agenda' && (
            <div className="space-y-6">
              ...
              <div className="relative w-full max-w-md">
                <Search className="w-4 h-4 text-[#7A7062] absolute left-4 top-1/2 -translate-y-1/2" />
                <input
                  type="text"
                  placeholder="Cari judul agenda atau lokasi..."
                  value={agendaSearchQuery}
                  onChange={(e) => setAgendaSearchQuery(e.target.value)}
                  className="w-full bg-white border border-[#E6E1D5] text-xs font-medium pl-10 pr-4 py-3 rounded-2xl focus:outline-none focus:border-[#2C4219] placeholder:text-[#9E9585] shadow-sm transition-all focus:shadow-md"
                />
              </div>
                      ...
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-black tracking-wider uppercase ${ag.status === 'Selesai' ? 'bg-gray-100 text-gray-600' : 'bg-[#E3EBD3] text-[#2C4219]'
                              }`}>
          ...
          {activeTab === 'moderation' && (
            <div className="space-y-6">
              ...
              {/* Moderation List (Simplified) */}
              <div className="space-y-3 pt-2">
                {filteredThreads.map((thr) => (
                  <div key={thr.id} className="bg-white p-4 rounded-2xl border border-[#E6E1D5] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-[#2C4219]/30 hover:bg-[#FAF6EE]/30 transition-all">
                    <div className="flex items-start gap-4 flex-1">
                      <div className="w-10 h-10 rounded-full bg-[#FAF6EE] shrink-0 border border-[#E6E1D5] overflow-hidden">
                        <img
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <span className="inline-block px-2 py-0.5 rounded text-[9px] font-black tracking-wider uppercase bg-[#E3EBD3] text-[#2C4219]">
                            {thr.category}
                          </span>
                          <span className="text-[10px] text-[#7A7062] font-semibold shrink-0">{thr.timeAgo || 'Baru saja'}</span>
                        </div>
                        <h4 className="font-bold text-[#2C4219] text-sm line-clamp-1">{thr.title}</h4>
                        <p className="text-[#7A7062] text-[11px] font-semibold mt-0.5">Oleh: {thr.authorName}</p>
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#2C4219] rounded-3xl p-6 sm:p-8 text-white relative overflow-hidden shadow-md">
                <div className="relative z-10 space-y-2">
                  <h1 className="font-title font-bold text-2xl sm:text-3xl text-white">
                    Selamat Datang, {currentUser?.firstName || currentUser?.name || 'Admin'}!
                  </h1>
                  <p className="text-white font-medium text-xs sm:text-sm max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div onClick={() => handleTabChange('informasi')} className="bg-white p-4 sm:p-5 rounded-3xl border border-[#E6E1D5] shadow-xs space-y-2 flex flex-col justify-between cursor-pointer hover:border-[#2C4219]/30 transition-colors">
                  ...
                  <p className="font-title font-black text-3xl text-[#2C4219]">{articles?.length || 4}</p>
                  <span className="text-[10px] text-emerald-700 font-bold flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Published & Siap Baca
                  </span>
                </div>

                <div onClick={() => handleTabChange('agenda')} className="bg-white p-4 sm:p-5 rounded-3xl border border-[#E6E1D5] shadow-xs space-y-2 flex flex-col justify-between cursor-pointer hover:border-[#2C4219]/30 transition-colors">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="font-title font-black text-3xl text-[#2C4219]">{agendaList?.length || 4}</p>
                  <span className="text-[10px] text-[#2C4219] font-bold flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#2C4219]" /> {agendaList?.filter(a => a.status === 'Belum dimulai').length || 4} Belum dimulai
                  </span>
                </div>

                <div className="bg-white p-4 sm:p-5 rounded-3xl border border-[#E6E1D5] shadow-xs space-y-2 flex flex-col justify-between cursor-pointer hover:border-[#2C4219]/30 transition-colors">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="font-title font-black text-3xl text-[#2C4219]">4</p>
                  <span className="text-[10px] text-amber-700 font-bold flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500" /> 2 Disematkan (Pinned)
                  </span>
                </div>

                <div onClick={() => handleTabChange('users')} className="bg-white p-4 sm:p-5 rounded-3xl border border-[#E6E1D5] shadow-xs space-y-2 flex flex-col justify-between cursor-pointer hover:border-[#2C4219]/30 transition-colors">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="font-title font-black text-3xl text-[#2C4219]">{dashboardStats?.totalUsers || members?.length || 6}</p>
                  <span className="text-[10px] text-emerald-700 font-bold flex items-center gap-1">
                    ...
                    {adminArticles && adminArticles.length > 0 ? (
                      adminArticles.slice(0, 3).map(art => (
                        <div key={art.id} className="p-3.5 rounded-2xl bg-[#FAF6EE] border border-[#E6E1D5] flex items-center justify-between gap-3">
                          ...
                          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ${art.status === 'Draft' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                            }`}>
                    ...
                    {threads && threads.length > 0 ? (
                      threads.slice(0, 3).map(thr => (
                        <div key={thr.id} className="p-3.5 rounded-2xl bg-[#FAF6EE] border border-[#E6E1D5] flex items-center justify-between gap-3">
                          ...
                          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ${thr.status === 'Aktif' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                            }`}>
                            {thr.status || 'Aktif'}
                          </span>
                        </div>
                    ...
                    <div key={plot.id} className="bg-white p-4 rounded-2xl border border-[#E6E1D5] shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-black tracking-wider uppercase ${plot.status === 'Aktif' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                            }`}>
                            {plot.status || 'Aktif'}
                          </span>
                        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(() => {
                  ...
                  return filtered.map((u) => {
                    ...
                    return (
                      <div key={u.id || u.email} className="bg-white p-3 sm:p-4 rounded-2xl border border-[#E6E1D5] shadow-xs flex items-center justify-between gap-4 hover:border-[#2C4219] transition-all">
                        <div className="flex items-center gap-3 min-w-0">
                          {u.avatar ? (
                            <img
                              src={u.avatar}
                              alt={userName}
                              className="w-10 h-10 rounded-2xl object-cover border border-[#E6E1D5] shrink-0"
                            />
                          ) : (
                            <div className="w-10 h-10 rounded-2xl bg-[#2C4219] text-[#A8B774] font-title font-black text-sm flex items-center justify-center shrink-0 shadow-xs">
                              {initials}
                            </div>
                          )}
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#FAF6EE] text-[#7A7062] border border-[#E6E1D5]">
                                {u.role || u.position || 'Anggota KWT'}
                              </span>
                              <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${isActive ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-xl transition-colors ${isActive ? 'text-amber-600 hover:bg-amber-50' : 'text-emerald-600 hover:bg-emerald-50'}`}
