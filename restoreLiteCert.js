...
const startTag = '{/* Agenda Selector */}';
...
if (startIdx !== -1 && endIdx !== -1) {
    const originalSelector = c.substring(startIdx + startTag.length, endIdx).trim();
    
    // Read liteSelector from a safe string
    const liteSelector = `
    ...
    c = c.substring(0, startIdx + startTag.length) + newBlock + c.substring(endIdx);
    ...
    console.log('Restored Lite Mode logic!');
} else {
